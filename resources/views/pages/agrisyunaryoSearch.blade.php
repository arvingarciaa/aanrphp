...
<div class="container section-margin results">
    <div class="row">
        <div class="col-sm-8 offset-2">
            <h1 class="font-weight-bold mb-3">Total Results: <b>{{$results->count()}}</b></h1>
            @if($results->count() == 0)
                <span class="text-muted">Sorry, there are no results found for <b>'{{$query}}'</b></span>
            @else
                ...
                @foreach($results as $result)
                    <div class="card mb-5 shadow rounded">
                        <div class="card-body">
                            ...
                            <h5 class="card-subtitle">{{$result->description}}</h5>
                        </div>
                        <img class="card-img-top" src="/storage/page_images/{{$result->image}}" alt="{{$result->title}}" style=" height: auto; 
                                                            width: 100%; 
                                                            max-height: 728px;">
